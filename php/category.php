...
$categoryTag = isset($_GET['tag']) ? $_GET['tag'] : '';

if ($categoryTag) {
    $categoryTag = htmlspecialchars($categoryTag);

    $posts_per_page = 5;

    $total_sql = "SELECT COUNT(*) FROM posts WHERE Tag LIKE ?";
    $stmt_total = $link->prepare($total_sql);
    $likeCategoryTag = "%$categoryTag%";
    $stmt_total->bind_param("s", $likeCategoryTag);
    $stmt_total->execute();
    ...
    $stmt->bind_param("sii", $likeCategoryTag, $posts_per_page, $offset);
    $stmt->execute();
        ...
        <main>
            <div class="content clearfix">
                <div class="main-content" id="post">
                    <h1 class="recent-post-title">Posts en Categoría: <?php echo htmlspecialchars($categoryTag); ?></h1>
                    <?php
                    if ($result && $result->num_rows > 0) {
                        ...
                        if ($current_page > 1) {
                            echo '<a href="?tag=' . urlencode($categoryTag) . '&page=' . ($current_page - 1) . '">&laquo; Anterior</a>';
                        }
                        ...
                        for ($i = 1; $i <= $total_pages; $i++) {
                            if ($i == $current_page) {
                                ...
                                echo '<a href="?tag=' . urlencode($categoryTag) . '&page=' . $i . '">' . $i . '</a>';
                            }
                        }

                        if ($current_page < $total_pages) {
                            echo '<a href="?tag=' . urlencode($categoryTag) . '&page=' . ($current_page + 1) . '">Siguiente &raquo;</a>';
                        }
                        echo '</div>';
                    } else {
                        echo '<p>No se encontraron posts para esta categoría.</p>';
                    }
                    ?>
