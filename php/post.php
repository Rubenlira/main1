...
<body>
    ...
    <div class="content clearfix">
            ...
            <div class="main-content single">
                ...
                if ($ID > 0) {
                    $sql = "SELECT titulo, imagen, contenido, imagen2, autor, fecha FROM $tablas WHERE ID = ?";
                    $stmt = $link->prepare($sql);
                    $stmt->bind_param("i", $ID);
                    $stmt->execute();
                    $stmt->bind_result($titulo, $imagen,  $contenido, $imagen2, $autor, $fecha);
                    $stmt->fetch();
                    $stmt->close();
                    ...
                    if ($titulo) {
                        ...
                        if ($imagen2) {
                            $imagePath = htmlspecialchars($imagen2); 
                            echo '<img src="' . $imagePath . '" alt="Imagen del post" class="center" style="height:auto; width=100%">';
                        } 
                    } else {
                ...
                <div class="post-content categoria">
                    <?php

                    if ($ID > 0) {
                        $sql = "SELECT tag FROM $tablas WHERE id = ?";
                        $stmt = $link->prepare($sql);
                        $stmt->bind_param('i', $ID);
                        $stmt->execute();
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                $tagName = htmlspecialchars($row['tag']);
                                echo "<h6 class='tag'>$tagName</h6>";
                            }
                        } else {
                            echo "<h6>No hay tags disponibles para este post.</h6>";
                        }
