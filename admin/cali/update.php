...
if (!isset($_SESSION['username'])) {
    ...
    header("refresh:3; url=https://sec263.rf.gd/login.php");
    exit();
}
...
if ($_POST) {
    ...
    if ($id > 0) {
        $query = "SELECT Imagen FROM $tablas5 WHERE ID = ?";
    ...
    if ($id > 0) {
        $queryUpdate = "UPDATE $tablas5 SET Titulo = ?, Autor = ?, Fecha = ?, Texto = ?, Imagen = ? WHERE ID = ?";
